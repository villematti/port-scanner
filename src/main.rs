        ...
        if let Ok(ipaddr) = IpAddr::from_str(&f) {
            ...
            if flag.contains("-h") || flag.contains("-help") && args.len() == 2 {
                ...
                let ipaddr = match IpAddr::from_str(&args[3]) {
                    Ok(s) => s,
                    Err(_) => return Err("not a calid IPADDR; must be Ipv4 or IPv6")
                };
...
fn main() {
    ...
    let argumetns = Arguments::new(&args).unwrap_or_else(|err| {
    ...
    let num_threads = argumetns.threads;
    let addr = argumetns.ipaddr;
    let (tx, rx) = channel();
